...
class InteractionIndex(object):
    def __init__(self, config, interaction_mapper, interaction_vectors):
        self.cf = config
    ...
    def knn_vector_query(self, vec, k=1):
        ...
        ret_interaction = [self.um.num_to_interaction(id) for id in ids]
        return ret_interaction, ids, distances

    def knn_interaction_query(self, interaction_str, k=1):
        return self.knn_idx_query(self.um.interaction_to_num(interaction_str), k=k)
